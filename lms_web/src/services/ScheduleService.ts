import { ApiResponseType, RoomType, SemesterType, SlotType } from "src/types/type";
...
export const getSemesters = async () : Promise<AxiosResponse<SemesterType[]>> => {
...
export const getAllRooms = async () : Promise<AxiosResponse<ApiResponseType<RoomType[]>>> => {
...
export const getAllSlots = async () : Promise<AxiosResponse<ApiResponseType<SlotType[]>>> => {
    return httpClient.get(API.SLOTS, {
        headers: {
            Authorization: `Bearer ${getToken()}`,
        }
    })
